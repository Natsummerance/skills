# Skills：技能集合（项目理解 + 信息改写 + 开发与 UX 优化）

四个互相独立、可按需调用的 AI Agent 技能（兼容 Codex / Claude Code 的 SKILL.md 规范）：`project-learning` 与 `document-learning` 覆盖“吃透一个新项目”的两种形态；`info-rewrite` 提供工程化信息改写流水线；`dev-ux-optimizer` 提供既有应用的全周期开发与 UX 优化（工程基线 → 增量里程碑 → UX 重塑 → 部署验证），并配套可执行的 agent 集群。
